...
const service = require('../services/excavacionService');
const router = express.Router();
const FOTOS = [{ name: 'foto_inicio', maxCount: 1 }, { name: 'foto_fondo', maxCount: 1 }, { name: 'foto_seguridad', maxCount: 1 }];
...
router.put('/:id', upload.fields(FOTOS), async (req, res) => { try { const r = await service.updateFromBody(req.params.id, req.body, req.files); if (!r.ok) return res.status(r.status).json({ error: r.error }); res.json({ mensaje: r.mensaje, id: r.id }); } catch (e) { res.status(500).json({ error: e.message }); } });
