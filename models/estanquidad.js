...
// ✅ PATCH - Subir PDF con mensajes claros y amigables (ÚNICA MODIFICACIÓN)
app.patch('/api/protocolos/:id/pdf', uploadPdf.single('pdf'), async (req, res) => {
  try {
    ...
    if (!protocolo) {
      return res.status(400).json({
        error: "❌ No se encontró el protocolo asociado a este ID. Asegúrate de haber guardado primero los datos del protocolo (presiona 'GUARDAR EN MONGO') y luego genera el PDF."
      });
    }
